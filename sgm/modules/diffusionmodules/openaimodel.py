...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(
        ...
        modulate_params: Optional[dict] = None,
    ):
...
class ResBlock(TimestepBlock):
    ...
    def _forward(self, x: th.Tensor, emb: th.Tensor) -> th.Tensor:
        if self.updown:
            ...
            h = self.in_layers(x)
            self.in_layers_features = h  # newly add
        ...
        if self.use_scale_shift_norm:
            ...
            h = h + emb_out
            h = self.out_layers(h)
            self.out_layers_features = h  # newly add
        ...
        **kwargs,
        ...
        for i, module in enumerate(self.input_blocks):  # only feature injection happens in the input_blocks
            ...
            if is_injected_step and "input" in modulate_params[
                ...
                is_injected_step_block = False
                if len(module) > 1 and "SpatialTransformer" in str(type(
        ...
        for i, module in enumerate(self.output_blocks):
            ...
            if is_modulate_step and i in modulate_block_idx and len(module) > 1 and "SpatialTransformer" in str(type(module[1])):
            ...
            if is_injected_step and len(module) > 1 and "SpatialTransformer" in str(
