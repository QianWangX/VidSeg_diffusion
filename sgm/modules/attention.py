    ...
    def forward(
        ...
        if self.checkpoint:
            def _forward_wrapper(x):
                return self._forward(x, context=context, is_modulate_step=is_modulate_step, 
                                     is_injected_step=is_injected_step,
                                     modulate_params=modulate_params)

            output = checkpoint(_forward_wrapper, x)
            ...
            return self._forward(is_modulate_step=is_modulate_step, is_injected_step=is_injected_step,
                                 modulate_params=modulate_params, **kwargs)
    ...
    def _forward(
        ...
        if is_injected_step:
            for k, v in modulate_params["injected_features_group"].items():
                if "spatial_self_attn_k" in k:
                    injected_k = v
                elif "spatial_self_attn_q" in k:
                    injected_q = v
                elif "spatial_self_attn_v" in k:
                    injected_v = v
            self.attn1_out = self.attn1(
                    self.norm1(x),
        ...
        if is_modulate_step and "self_attn" in modulate_params["modulate_attn_type"]:
            ...
            for i, mask in enumerate(masks):
                if i in modulate_params["modulate_block_frames_group"] and i in modulate_params["modulate_layer_frames_group"] and \
                    ...
                    self.attn1_out[i + num_masks] = self.attn1_out[i + num_masks] + \
                        modulate_lambda * mask[:, None]
                    if modulate_params["modulate_uc"]:
                        self.attn1_out[i] = self.attn1_out[i] + \
                            modulate_lambda * mask[:, None]
        x = (
            self.attn1_out
            + x
        ...
        if is_injected_step:
            for k, v in modulate_params["injected_features_group"].items():
                if "spatial_cross_attn_k" in k:
                    injected_k = v
                elif "spatial_cross_attn_q" in k:
                    injected_q = v
                elif "spatial_cross_attn_v" in k:
                    injected_v = v
            self.attn2_out = self.attn2(
        ...
        # compute the mean and sigma of the 1st dimention of self.attn2_out
        # attn_mean_ref = torch.mean(self.attn2_out, dim=1)[:, None, :]
        # attn_var_ref = torch.var(self.attn2_out, dim=1)[:, None, :]
        ...
        if is_modulate_step and "cross_attn" in modulate_params["modulate_attn_type"]:
            ...
            for i, mask in enumerate(masks):
                ...
                if i in modulate_params["modulate_block_frames_group"] and i in modulate_params["modulate_layer_frames_group"] and \
                    ...
                    self.attn2_out[i + num_masks] = self.attn2_out[i + num_masks] + \
                        modulate_lambda * mask[:, None]
                    if modulate_params["modulate_uc"]:
                        self.attn2_out[i] = self.attn2_out[i] + \
                            modulate_lambda * mask[:, None]
        
            # attn_mean = torch.mean(self.attn2_out, dim=1)[:, None, :]
            # attn_var = torch.var(self.attn2_out, dim=1)[:, None, :]
            
            # self.attn2_out = (self.attn2_out - attn_mean) * torch.sqrt(attn_var_ref / (attn_var + 1e-8)) + attn_mean_ref
        ...
        if is_modulate_step and "ff_out" in modulate_params["modulate_attn_type"]:
            ...
            for i, mask in enumerate(masks):
                ...
                if i in modulate_params["modulate_block_frames_group"] and i in modulate_params["modulate_layer_frames_group"] and \
                    ...
                    self.ff_out[i + num_masks] = self.ff_out[i + num_masks] + \
                        modulate_lambda * mask[:, None]
                    if modulate_params["modulate_uc"]:
                        self.ff_out[i] = self.ff_out[i] + \
                            modulate_lambda * mask[:, None]
