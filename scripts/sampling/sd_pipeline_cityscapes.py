...
def sample(
    ...
    feature_folder: str = "features_outputs_image_cityscapes",
    ...
    def load_feature_masks(masks_path, mask_id, num_frames=14, feature_timestep="24", mask_source="kmeans", modulate_block_idx=7,
    ...
    sub_seq_end_idx = min(sub_seq_start_idx + num_sub_seq, len(all_sub_seq_path))            
    all_sub_seq_path = all_sub_seq_path[sub_seq_start_idx:sub_seq_end_idx]
    print(f"Total number of sub sequences: {len(all_sub_seq_path)}")
    ...
    for sub_seq_id, sub_seq_path in enumerate(all_sub_seq_path):
        ...
        for frame_id, frame in enumerate(sub_seq_path):
            ...
            # center crop the images from [2048, 1024] to [512, 256]
            ...
            if os.path.exists(os.path.join(input_gt_path, frame_name.replace("leftImg8bit", "gtFine_labelIds") + ".png")):
                frame_gt_name_list.append(frame_name)
                frame_gt_img = Image.open(frame)
                if frame_gt_img.mode == "RGBA":
                    frame_gt_img = frame_gt_img.convert("RGB")
                    ori_w, ori_h = frame_gt_img.size

                    if ori_h % 64 != 0 or ori_w % 64 != 0:
                        width, height = map(lambda x: x - x % 64, (ori_w, ori_h))
                        frame_gt_img = frame_gt_img.crop((width // 4, height // 4, width // 4 + 512, height // 4 + 256))
                frame_gt_img_list.append(frame_gt_img)
        ...
        for batch_id in range(batch_start_idx, num_batches):
            ...
            with torch.no_grad():
                with torch.autocast(device):                
                    ...
                    if batch_id == -1:
                        gt_mask_name = frame_name_list_batch[0].replace("leftImg8bit", "gtFine_labelIds")
...
def load_model_from_config(config, device, ckpt=None, verbose=True):
    ...
    # config.model.params.sampler_config.params.num_steps = 25
    # config.model.params.sampler_config.params.guider_config.params.num_frames = (1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--modulate_timestep", type=str, default="20", help="selected modulate timestep")
    parser.add_argument("--modulate_schedule", type=str, default="constant", help="modulate lambda schedule")
    parser.add_argument("--modulate_lambda_start", type=float, default=10.0, help="modulate lambda start")
    parser.add_argument("--modulate_lambda_end", type=float, default=10.0, help="modulate lambda end")
    ...
    parser.add_argument("--feature_folder", type=str, default="features_outputs_image_cityscapes", help="feature folder path")
    ...
    parser.add_argument("--num_sub_seq", type=int, default=100, help="number of sub sequences to run")
